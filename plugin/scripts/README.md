...
| `inject-vyasa-root.sh`   | SessionStart hook: replaces `@@VYASA_ROOT@@` in agent files with the real plugin path; records injected filenames in `agents/.vyasa-root-injected` (one per line) so new agents are picked up automatically on subsequent sessions |
